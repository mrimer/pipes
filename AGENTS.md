...
└── campaignEditor/
    ...
    ├── campaignBrowserSection.ts # Campaign-list/campaign-detail/chapter-detail screens + CRUD actions (create/add/delete/duplicate/reorder/move) — collaborator extracted from index.ts; navigation state (_screen/_activeCampaignId/_activeChapterIdx/_activeLevelIdx) stays on CampaignEditor
...
- **`guid` is the one cross-install "is this the same campaign" identifier, shared by both import paths.** `CampaignService._findMatchingCampaignIndex()` (used by both `parseImport`/`acceptImport` for full-campaign import and, via `getCampaignByGuid()`, by `mergeTextPack()` for text-pack import) prefers guid equality and falls back to `id` equality only when either side lacks a guid — `id` alone is too weak (collision-prone, not a real UUID) to trust for this across two separate installs. Don't reintroduce a third id-like scheme for a future cross-install matching need; extend/reuse `guid`.
...
| Campaign-list/campaign-detail/chapter-detail screens, campaign CRUD actions | `campaignEditor/campaignBrowserSection.ts` |
