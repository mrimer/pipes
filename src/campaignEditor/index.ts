...
export class CampaignEditor {
  ...
  // ── Chapter map editor state ──────────────────────────────────────────────
  /** Number of rows in the chapter map grid being edited. */
  private _chapterEditRows = 3;
  /** Number of columns in the chapter map grid being edited. */
  private _chapterEditCols = 6;
  /** The chapter map grid being edited (row-major; null = empty/fillable cell). */
  private _chapterEditGrid: (TileDef | null)[][] = [];
  /** Active palette selection for the chapter map editor. */
  private _chapterPalette: EditorPalette = PipeShape.Source;
  /** Active tile parameters for the chapter map editor. */
  private _chapterParams: TileParams = { ...DEFAULT_PARAMS };
  /** Canvas element for the chapter map editor grid. */
  private _chapterCanvas: HTMLCanvasElement | null = null;
  /** 2D context for the chapter map editor canvas. */
  private _chapterCtx: CanvasRenderingContext2D | null = null;
  /** Current hover cell position in the chapter map editor. */
  private _chapterHover: { row: number; col: number } | null = null;
  /** Drag state for moving chambers within the chapter map editor. */
  ...
  /** True while a paint-drag is active in the chapter map editor. */
  private _chapterPaintDragActive = false;
  /** True while a right-erase-drag is active in the chapter map editor. */
  private _chapterRightEraseDragActive = false;
  /** True when the contextmenu event following a right-drag should be suppressed. */
  private _chapterSuppressContextMenu = false;
  /** Undo/redo history snapshots for the chapter map editor. */
  private _chapterHistory: EditorSnapshot[] = [];
  /** Current position in chapter editor history (index into _chapterHistory). */
  private _chapterHistoryIdx = -1;
  /** Index of the level (within the chapter) selected for placement; null = none. */
  private _chapterSelectedLevelIdx: number | null = null;
  /** Bound window mouseup handler for chapter map editor drag; stored so it can be removed. */
  private _chapterWindowMouseUpHandler: ((e: MouseEvent) => void) | null = null;
  /** Container element for the chapter map editor layout (used to measure canvas space). */
  private _chapterEditorMainLayout: HTMLElement | null = null;
  ...
  private _showChapterDetail(): void {
    ...
    content.appendChild(this._buildChapterMapSection(campaign, chapter, isOfficial));
  ...
  private _initChapterGridState(chapter: ChapterDef): void {
    if (chapter.grid && chapter.rows && chapter.cols) {
      ...
      const rows = CampaignEditor.CHAPTER_DEFAULT_ROWS;
      const cols = CampaignEditor.CHAPTER_DEFAULT_COLS;
  ...
  private _buildChapterMapSection(campaign: CampaignDef, chapter: ChapterDef, isOfficial: boolean): HTMLElement {
    ...
    const undoBtn = this._btn('↩ Undo', '#2a2a4a', '#aaa', () => this._chapterUndo(campaign, chapter));
    undoBtn.id = 'chapter-undo-btn';
    midToolbar.appendChild(undoBtn);
    const redoBtn = this._btn('↪ Redo', '#2a2a4a', '#aaa', () => this._chapterRedo(campaign, chapter));
    redoBtn.id = 'chapter-redo-btn';
    midToolbar.appendChild(redoBtn);

    midToolbar.appendChild(this._btn('✔ Validate', '#16213e', '#7ed321', () => {
