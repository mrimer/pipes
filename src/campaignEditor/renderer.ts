...
/**
 * Render the chapter map canvas (used on the chapter map screen, not the editor).
 *
 * @param ctx            2D context to draw on.
...
export function renderChapterMapCanvas(
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      if (def.shape === PipeShape.Chamber && def.chamberContent === 'level') {
        ...
        _drawEditorTileWithLevel(ctx, x, y, def, levelDefs, progress);
        // Water-fill overlay
  ...
  // Pass 3: pipe tiles with water fill overlay
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      const def = grid[r]?.[c] ?? null;
      if (def === null || !PIPE_SHAPES.has(def.shape)) continue;
      ...
      if (isFilled) {
        ctx.save();
        ctx.fillStyle = 'rgba(74,160,255,0.25)';
        ctx.fillRect(x, y, CELL, CELL);
        ctx.restore();
      }
      ctx.strokeStyle = '#2a3a5e';
